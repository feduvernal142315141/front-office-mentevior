 ...
 *
 * A diferencia de Clinical Monthly, el backend recibe el id en el **path**
 * (`/reports/monthly-supervision/{id}/preview`), no como query param.
 *
 * Example:
...
export async function GET(
  ...
  const monthlySupervisionId = req.nextUrl.searchParams.get("monthlySupervisionId")
  if (!monthlySupervisionId) {
    return NextResponse.json({ error: "monthlySupervisionId is required" }, { status: 400 })
  }
  ...
  const fallbackName = decodeURIComponent(rawFileName || "Monthly Supervision.pdf") || "Monthly Supervision.pdf"

  const upstreamUrl = `${apiBase}/reports/monthly-supervision/${encodeURIComponent(monthlySupervisionId)}/preview`
  ...
  if (upstreamType.includes("application/json") || upstreamType.includes("text/")) {
    // The contract returns { fileBase64 }
