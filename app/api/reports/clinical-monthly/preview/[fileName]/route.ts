...
export async function GET(
  ...
  const clinicalMonthlyId = req.nextUrl.searchParams.get("clinicalMonthlyId")
  if (!clinicalMonthlyId) {
    return NextResponse.json({ error: "clinicalMonthlyId is required" }, { status: 400 })
  }
  ...
  const fallbackName = decodeURIComponent(rawFileName || "Clinical Monthly.pdf") || "Clinical Monthly.pdf"

  const upstreamUrl = `${apiBase}/reports/clinical-monthly/preview?clinicalMonthlyId=${encodeURIComponent(clinicalMonthlyId)}`
