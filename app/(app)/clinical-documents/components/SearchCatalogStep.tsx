...
import { useClinicalDocumentsCatalog } from "@/lib/modules/clinical-documents/hooks/use-clinical-documents-catalog"
import { bulkCreateClinicalDocuments } from "@/lib/modules/clinical-documents/services/clinical-documents.service"
import type { ClinicalDocumentCatalogItem } from "@/lib/types/clinical-document.types"
...
interface GroupedDocuments {
  groupName: string
  documents: ClinicalDocumentCatalogItem[]
}
...
export function SearchCatalogStep({ onSuccess, onClose }: SearchCatalogStepProps) {
  ...
  const { data, isLoading } = useClinicalDocumentsCatalog("CLINICAL")
  ...
  const filteredDocuments = catalogDocuments.filter((doc: ClinicalDocumentCatalogItem) => {
    if (!searchTerm.trim()) return true
    return doc.name.toLowerCase().includes(searchTerm.toLowerCase())
  })

  const groupedDocuments = useMemo((): GroupedDocuments[] => {
    const groups = new Map<string, ClinicalDocumentCatalogItem[]>()
  ...
  const toggleSelectAll = () => {
    if (selectedIds.size === filteredDocuments.length && filteredDocuments.length > 0) {
      ...
      setSelectedIds(new Set(filteredDocuments.map((c: ClinicalDocumentCatalogItem) => c.id)))
    }
  }
  ...
  const handleSubmit = async () => {
    ...
    if (selectedIds.size === 1) {
      ...
      const selectedDocument = catalogDocuments.find((c: ClinicalDocumentCatalogItem) => selectedIds.has(c.id))
      if (selectedDocument) {
        onClose()
        ...
        router.push(`/clinical-documents/create?${params.toString()}`)
      }
    } else {
      ...
      try {
        ...
        await bulkCreateClinicalDocuments({ catalogIds: selectedCatalogIds, documentCategory: "CLINICAL" })
  ...
  const renderDocumentItem = (document: ClinicalDocumentCatalogItem) => {
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="p-6 border-b border-gray-200">
        <p className="text-gray-600 mt-1">Find and select clinical documents</p>
      </div>
