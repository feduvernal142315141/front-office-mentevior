...
import { Sheet, Loader2, AlertTriangle, Maximize2, Minimize2, X, FileDown, Pencil } from "lucide-react"
          ...
          {activeItem && (
            <div ref={exportRef} className="rounded-2xl border border-slate-200 bg-white shadow-sm p-6">
              <div className="flex items-start justify-between gap-4 mb-4">
                <button
                  type="button"
                  className="group flex min-w-0 items-center gap-2 rounded-lg px-2 py-1 -ml-2 transition-colors hover:bg-slate-50"
                  onClick={() => {
                    if (!activeCategoryId) return
                    onNavigateToItem?.({
                ...
                >
                  <span className="truncate text-sm font-semibold text-slate-800">{activeItem.itemName}</span>
                  <Pencil className="h-3.5 w-3.5 shrink-0 text-slate-400 transition-colors group-hover:text-[#037ECC]" />
                </button>
