...
import { getClientServicePlanById } from "@/lib/modules/client-service-plan/services/client-service-plan.service"
import type { ClientServicePlan } from "@/lib/types/client-service-plan.types"

interface UseClientServicePlanByIdResult {
...
export function useClientServicePlanById(id: string): UseClientServicePlanByIdResult {
  ...
  const fetch = useCallback(async () => {
    ...
    try {
      const plan = await getClientServicePlanById(id)
