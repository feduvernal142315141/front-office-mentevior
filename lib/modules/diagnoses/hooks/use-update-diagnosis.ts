...
interface UseUpdateDiagnosisReturn {
  update: (diagnosisId: string, data: UpdateDiagnosisDto) => Promise<string | null>
  isLoading: boolean
  error: string | null
}
...
export function useUpdateDiagnosis(): UseUpdateDiagnosisReturn {
  ...
  const update = async (diagnosisId: string, data: UpdateDiagnosisDto): Promise<string | null> => {
    setIsLoading(true)
    setError(null)

    try {
      const updatedDiagnosisId = await updateDiagnosis(diagnosisId, data)
