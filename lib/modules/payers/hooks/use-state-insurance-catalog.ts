...
interface UseStateInsuranceCatalogReturn {
...
export function useStateInsuranceCatalog(): UseStateInsuranceCatalogReturn {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        const result = await getPayersService().getFlMedicaidCatalog()
