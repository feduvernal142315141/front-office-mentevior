...
interface UseCreateFromPrivateInsuranceReturn {
  createFromCatalog: (ids: string[]) => Promise<boolean>
  isLoading: boolean
}

export function useCreateFromPrivateInsurance(): UseCreateFromPrivateInsuranceReturn {
  ...
  const createFromCatalog = async (ids: string[]): Promise<boolean> => {
    ...
    try {
      await getPayersService().createFromPrivateInsurance(ids)
