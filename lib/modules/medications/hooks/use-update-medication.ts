...
interface UseUpdateMedicationReturn {
  update: (medicationId: string, data: UpdateMedicationDto) => Promise<string | null>
  isLoading: boolean
  error: string | null
}
...
export function useUpdateMedication(): UseUpdateMedicationReturn {
  ...
  const update = async (medicationId: string, data: UpdateMedicationDto): Promise<string | null> => {
    setIsLoading(true)
    setError(null)

    try {
      const updatedMedicationId = await updateMedication(medicationId, data)
      toast.success("Medication updated successfully")
      return updatedMedicationId
    } catch (err) {
