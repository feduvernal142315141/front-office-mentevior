...
interface UseUpdateAppointmentConfigReturn {
  update: (data: UpsertAppointmentConfigDto) => Promise<AppointmentConfig | null>
  isLoading: boolean
}

export function useUpdateAppointmentConfig(): UseUpdateAppointmentConfigReturn {
  const [isLoading, setIsLoading] = useState(false)

  const update = async (data: UpsertAppointmentConfigDto): Promise<AppointmentConfig | null> => {
    try {
      setIsLoading(true)
