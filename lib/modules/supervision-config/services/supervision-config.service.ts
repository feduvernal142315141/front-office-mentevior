...
export async function upsertSupervisionConfig(data: UpsertSupervisionConfigDto): Promise<SupervisionConfig> {
  const response = await servicePutSilent<UpsertSupervisionConfigDto, SupervisionConfig>("/supervision-config", data)

  if (response.status !== 200 && response.status !== 201) {
    throw new Error(getApiErrorMessage(response?.data, "Failed to save supervision configuration"))
  }
